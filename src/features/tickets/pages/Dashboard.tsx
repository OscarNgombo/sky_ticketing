...
function Dashboard({ user }: TicketPageProps) {
  ...
  const visibleTickets: TicketRecord[] =
    !isVendor && currentCompany
      ? ticketsDataRaw.filter(
          (t: TicketRecord) =>
            (t.company && t.company === currentCompany) ||
            (typeof t.source === "string" && t.source?.includes(currentCompany))
        )
      : ticketsDataRaw;
