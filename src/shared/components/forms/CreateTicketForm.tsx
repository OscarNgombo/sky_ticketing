...
import {
  ...
  UnderlineIcon,
  VideoIcon,
} from "../../icons/TicketEditorIcons";
import "./CreateTicketForm.css";
import { VideoExtension } from "../../../utils/tiptap/extensions/VideoExtension.tsx";
...
const CreateTicketForm: React.FC<CreateTicketFormProps> = ({
  ...
  const saveTicketToLocalStorage = async () => {
    ...
    const encrypted = localStorage.getItem(TICKET_STORAGE_KEY);
    if (encrypted) {
      try {
        ...
        if (decrypted) {
          existingTickets = JSON.parse(decrypted);
        }
      } catch {
        // Failed to decrypt or parse tickets, start with empty array
      }
    }

    // Encrypt and save
    const newTickets = [...existingTickets, newTicket];
    const encryptedTickets = CryptoJS.AES.encrypt(
      JSON.stringify(newTickets),
      TICKET_SECRET
    ).toString();
    localStorage.setItem(TICKET_STORAGE_KEY, encryptedTickets);
    return newTicket;
  };
  ...
  const handleCreate = async () => {
    ...
    if (isValid && hasFile) {
      const newTicket = await saveTicketToLocalStorage();
      ...
      if (!hasFile) {
        setErrors((prev) => ({
          ...prev,
          attachment: "At least one file must be attached.",
        }));
      }
      console.log("Form is invalid or no file attached.");
    }
  };
