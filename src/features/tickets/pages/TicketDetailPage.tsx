...
import { getCurrentUser, decodeTicketId } from "../../../utils/auth";
...
function TicketDetailPage() {
  const params = useParams({
    from: "/_authenticated/tickets/$ticketId" as const,
  });
  const rawParam = params?.ticketId ?? "";
  const ticketId = decodeTicketId(rawParam);

  const [ticket, setTicket] = useState<StoredTicket | null>(null);
  const [editState, setEditState] = useState<Partial<StoredTicket> | null>(
    null
  );
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      setEditState((s) =>
        s?.description === html ? s : { ...(s || {}), description: html }
      );
    },
  });
    ...
    try {
      ...
      if (found) {
        if (!isVendor) {
          ...
          const ticketCompany = (found as StoredTicket).company || "";
          const companyMatches =
            (ticketCompany && ticketCompany === userCompany) ||
            (found.source && found.source.includes(userCompany));
          if (!companyMatches) {
          ...
          <div className="ticket-detail-grid">
            <div className="ticket-detail-left">
              ...
              <label>
                Category
                <input
                  value={editState?.mainCategory ?? ticket.mainCategory}
                  onChange={(e) =>
                    setEditState((s) => ({
                      ...(s || {}),
              ...
              <label>
                Sub Category
                <input
                  value={editState?.subCategory ?? ticket.subCategory}
                  onChange={(e) =>
                    setEditState((s) => ({
                      ...(s || {}),
              ...
              <label>
                Problem
                <input
                  value={editState?.problem ?? ticket.problem}
                  onChange={(e) =>
                    setEditState((s) => ({
                      ...(s || {}),
              ...
              <div style={{ marginTop: 12 }}>
                <button
                  onClick={async () => {
                    ...
                    const filesToValidate = (editState?.files ??
                      ticket.files ??
                      []) as StoredFile[];
                    const MAX_FILES = 5;
                    const MAX_SIZE_MB = 2;
                    ...
                    for (const f of filesToValidate) {
                      if (f.size > MAX_SIZE_MB * 1024 * 1024) {
                        alert(
                          `File "${f.name}" exceeds the maximum size of ${MAX_SIZE_MB}MB.`
                        );
                        return;
                      }
                    ...
                    try {
                      ...
                      const parsed: StoredTicket[] = JSON.parse(
                        decrypted || "[]"
                      );
                      ...
                      if (idx === -1) return;
                      const statusToSave = isVendor
                        ? status
                        : parsed[idx].status ?? "Open";
                      parsed[idx] = {
                        ...parsed[idx],
                        ...(editState || {}),
